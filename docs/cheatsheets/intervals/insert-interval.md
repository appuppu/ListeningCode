...
public int[][] insert(int[][] intervals, int[] newInterval) {
    // 結果を格納する可変長リスト。サイズが事前にわからないのでArrayListを使う
    List<int[]> res = new ArrayList<>();
    // 走査位置を追跡する変数
    int i = 0;
    // 区間の総数を変数に保存してループ条件で繰り返し参照する
    int n = intervals.length;

    // フェーズ1: newIntervalより完全に前にある区間をそのまま追加する
    // 条件: 既存区間の終端 < newIntervalの始端 → 重複しない
    ...
    // 条件: 既存区間の始端 <= newIntervalの終端 → 重複している
    while (i < n && intervals[i][0] <= newInterval[1]) {
        // 始端は小さい方を取る（マージ範囲を左に拡大する可能性がある）
        newInterval[0] = Math.min(newInterval[0], intervals[i][0]);
        // 終端は大きい方を取る（マージ範囲を右に拡大する可能性がある）
    ...
    // ArrayListをint[][]に変換して返す。new int[0][]はtoArrayの型ヒント
