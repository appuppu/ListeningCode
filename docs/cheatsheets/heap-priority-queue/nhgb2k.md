...
public int lastStoneWeight(int[] stones) {
    // Collections.reverseOrder() でMax-Heap（最大値が先頭）を作成する
    // デフォルトのPriorityQueueはMin-Heapなので、コンパレータで降順に反転させる
    PriorityQueue<Integer> pq =
        new PriorityQueue<>(Collections.reverseOrder());

    // 全ての石をヒープに追加する。全要素の追加後、ヒープが最大値を先頭に管理する
    ...
    while (pq.size() >= 2) {
        ...
        int a = pq.poll();  // 最も重い石を取り出す
        int b = pq.poll();  // 次に重い石を取り出す
