...
class Twitter {
    // グローバルなタイムスタンプ。すべてのユーザーのツイート間で投稿順序を一意に識別する（単調増加）
    int time = 0;
    ...
    void postTweet(int userId, int tweetId) {
        ...
        // time++でグローバルに単調増加するため、すべてのユーザー間で投稿順序を正しく比較できる
    ...
    void unfollow(int followerId, int followeeId) {
        // フォロー関係が存在する場合のみ、followeeIdをSetから削除する。存在しない場合は何もしない
    ...
    List<Integer> getNewsFeed(int userId) {
        ...
        // 各ユーザーの最新ツイート（リスト末尾）をヒープに追加する
        // ユーザーIDとインデックスを含めるのは、取り出し後にそのユーザーの次のツイートを辿るため
        for (int uid : users) {
            if (!tweets.containsKey(uid)) continue;
            List<int[]> t = tweets.get(uid);
            int idx = t.size() - 1;
            int[] tw = t.get(idx);
            // [タイムスタンプ, ツイートID, ユーザーID, リスト内インデックス] の4要素配列
            pq.offer(new int[]{tw[0], tw[1], uid, idx});
        }

        // ヒープから最大10件を取り出す（ヒープが空になるか10件に達したら早期終了）
        ...
        while (!pq.isEmpty() && res.size() < 10) {
            ...
            int idx = top[3] - 1;
            // そのユーザーにまだツイートがあればヒープに追加する（インデックスが負なら全て処理済み）
            if (idx >= 0) {
