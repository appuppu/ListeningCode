# Simulating a Last Stone Weight Game — 돌을 2개씩 부딪혀 마지막에 남는 돌의 무게를 구하기

## 문제의 본질

정수 배열 `stones`가 주어진다. 매번 **가장 무거운 2개의 돌**을 꺼내어 부딪힌다. 두 돌의 무게가 같으면 둘 다 소멸하고, 다르면 가벼운 쪽이 소멸하고 무거운 쪽은 차이만큼의 무게로 줄어든다. 이 연산을 돌이 1개 이하가 될 때까지 반복하고, 마지막에 남은 돌의 무게를 반환한다. 돌이 남지 않으면 0을 반환한다.
...
1. **매번의 연산에 필요한 것은 최댓값 2개이다**: 돌을 부딪히는 규칙에서는 항상 가장 무거운 2개의 돌을 선택한다. 즉 "현재 집합에서 최댓값을 2번 꺼내는" 연산을 반복하는 문제이다
...
4. **모든 돌을 힙에 넣는다**: 배열 `stones`의 모든 요소를 PriorityQueue에 추가한다. 이로써 힙이 최댓값을 관리할 준비가 완료된다
5. **돌이 2개 이상 있는 동안 부딪히는 연산을 반복한다**: 힙에서 `poll()`로 최댓값을 2번 꺼내고, 차이가 0이 아니면 차이를 힙에 `add()`로 되돌린다. 차이가 0이면 아무것도 되돌리지 않는다(둘 다 소멸)
6. **마지막 상태를 판정하여 반환한다**: 루프 종료 후 힙이 비어 있으면 모든 돌이 소멸했으므로 0을 반환한다. 힙에 1개 남아 있으면 그 돌의 무게를 `poll()`로 꺼내어 반환한다

## 전제 지식

### PriorityQueue(우선순위 큐)란

요소를 추가하면 내부에서 자동으로 순서가 관리되며, `poll()`로 항상 가장 우선순위가 높은 요소를 꺼낼 수 있는 데이터 구조이다. 내부 구현은 힙(이진 힙)이며, 추가와 추출 모두 O(log n)으로 동작한다.
...
maxHeap.poll();       // 최댓값 7을 꺼내어 반환한다
maxHeap.poll();       // 다음 최댓값 3을 꺼내어 반환한다
```
...
| | 값 |
|---|---|
| Time | O(n log n) — 최대 n번의 부딪히는 연산이 있으며, 각 연산에서 힙의 추출·추가에 O(log n)이 소요된다 |
...
public int lastStoneWeight(int[] stones) {
    // Collections.reverseOrder()를 비교자로 지정하여, poll()이 최댓값을 반환하는 Max-Heap을 생성한다
    PriorityQueue<Integer> pq =
        new PriorityQueue<>(Collections.reverseOrder());

    // 모든 돌을 힙에 추가한다. 완료되면 힙이 최댓값을 선두에서 관리하는 상태가 된다
    for (int s : stones) pq.add(s);

    // 돌이 2개 이상 있는 동안, 가장 무거운 2개를 부딪히는 연산을 반복한다
    while (pq.size() >= 2) {
        // poll()을 2번 호출하여, 가장 무거운 돌과 그 다음으로 무거운 돌을 꺼낸다
    ...
    // 힙이 비어 있으면 모든 돌이 소멸했으므로 0을, 남아 있으면 그 돌의 무게를 반환한다
